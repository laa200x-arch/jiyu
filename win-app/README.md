...
| 消息 | 实时聊天（文本/图片/视频/语音/拍照确认）、风控拦截、加载更早消息、聊天记录同步开关、新消息弹窗 + 未读红点 + 任务栏闪烁 |
| **宠物护理**（旧巡六迁移 → 收费订单） | 🐾 底部导航第 5 个 Tab：宠物档案（类型/品种/年龄/性别/绝育/体重/行为/家中反应/照片/备注 + 完整校验）、7 种护理服务（含定价 ¥15-50）、**发起订单**（指定认识的看护人 / 发布到动态区等有资历者接单）、订单列表与状态、平台佣金 10%（其余归服务人员） |
