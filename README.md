...
| 项目 | 要求 |
|---|---|
| 格式 | **单个 HTML 文件**，CSS/JS 全部内联（自包含） |
| 大小 | ≤ 512KB |
| 禁止 | 外链脚本（`<script src>`）、外链样式（`<link>`）、内嵌 iframe、外部网络请求 |
| 运行环境 | 沙箱 iframe / WKWebView 加载，无网络权限；请使用内联数据实现功能 |
| 示例 | `server/src/snake-app.html`（贪吃蛇，预置在市场） |
| 发布 | Windows 端：消息页 → 小程序 → 发布（选择 .html 文件 + 名称/简介） |
